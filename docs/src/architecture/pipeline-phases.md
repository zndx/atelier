...
| 5 | **`VALIDATING`** | ML re-validation: CatBoost (fit-to-LLM during the loop) and the registered SVM — the ModernBERT mean-pool factorized fully-hierarchical NHSVM, trained offline on the synth corpus and emitting user codes natively (no runtime vocabulary alignment) — score the same columns independently of the LLM. Per-column DST mass with conflict K is computed under the parent-aware frame. The disagreement set — driven primarily by belief-gap `Pl − Bel`, with K and coverage as secondary signals — feeds the next iteration's revisit batch. The loop exits when convergence criteria are satisfied; otherwise it re-enters `LLM_SWEEP`. | `state.ml_prediction`, `state.ml_belief`, `state.ml_plausibility`, `state.ml_conflict`, the next iteration's disagreement list |
| 6 | **`CLASSIFYING`** | Final per-column DST evidence fusion. Up to six evidence sources combine: `name_match`, `pattern`, `maxsim`, `llm`, `catboost`, `svm`. Each produces a mass function over the parent-aware frame; per-column predicted code, belief, plausibility, and conflict are computed here. | `classifications: list[dict]` (each entry shaped as `classifications.json` rows) |
...
| `LLM_SWEEP` ⇄ `VALIDATING` | `svm.pkl` + `svm.classes.json` | The registered SVM — ModernBERT factorized fully-hierarchical NHSVM, trained offline on the synth corpus and emitting user codes natively. Persisted for Extend runs. (The legacy filename `svm_frontier.pkl` was the excised M9 in-loop SVM-on-LLM-labels retrain; current runs write `svm.pkl`, and the legacy name is read only when re-registering pre-excision run directories.) |
