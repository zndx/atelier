...
Generated by `scripts/generate_sample_source.py`. Ground truth for
the Sample source fixture is committed in
`data/sample/ground_truth.json` (scope: fixture-only, for OOTB demo
and unit tests).

For UAT / production evaluation, the authoritative ground truth lives
at `build/meta-tagging-clean/ground_truth.csv` (gitignored) — built
by `scripts/parity/build_authoritative_ground_truth.py` from direct
reference-column evidence plus name-index lookup with
Ontology > Annotation > Common Names priority.  UAT's own
classification outputs are *provisional predictions*, not ground
truth, and are scored against this authoritative reference at
...
| `data/sample/ground_truth.json` | 316-entry Sample-source fixture ground truth |
| `build/meta-tagging-clean/ground_truth.csv` (gitignored) | Authoritative UAT-corpus GT reference |
