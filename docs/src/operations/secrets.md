...
can stand up a working instance by entering only three environment
variables — their two AWS Bedrock credentials plus a single age
private key that unlocks everything else.
...
subagent model IDs, etc. Most of those values are identical across
every deployment of the same Atelier release — only the AWS credentials
are operator-specific. Rather than documenting a long checklist for
every customer, we **encrypt the defaults** into the repository with
...
Set three environment variables on the CAI Application, then start it:
...
On startup, `bin/start-app.sh` detects `.env.cai.enc`, decrypts it with
the age key you provided, sources the values into the environment,
and HOCON's `${?VAR}` substitution picks them up before the gRPC
server binds. No per-customer checklist to maintain.
...
sops updatekeys .env.cai.enc                      # re-encrypt for new recipient
...
- `.env.cai.enc` — encrypted defaults (committed)
- `bin/start-app.sh` — startup decrypt + source
- `justfile` (`decrypt-secrets`, `encrypt-secrets`) — maintainer helpers
- `devenv.nix` — provides `sops` + `age` in the dev shell
