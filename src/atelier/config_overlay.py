...
SETTINGS_METADATA: dict[str, dict[str, Any]] = {
    ...
    "classify_bootstrap_sweep_deadline_s": {
        ...
        "description": "Wall-clock ceiling on the LLM sweep phase; raises SweepDeadlineError and transitions FSM to ERROR on breach",
        "group": "convergence",
        "type": "float",
        "min": 300.0,
        "max": 7200.0,
        "step": 60.0,
        "default": 1800.0,
        "caption_template": "Abort the sweep after {value}s of wall-clock — stops a blackholed endpoint from chewing through hours of halving-retry timeouts before anyone notices.",
    },
