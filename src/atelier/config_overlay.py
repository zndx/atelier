...
SETTINGS_METADATA: dict[str, dict[str, Any]] = {
    ...
    "classify_cautious_review_bel_threshold": {
        ...
        "step": 0.05,
        "default": 0.80,
        "caption_template": "Review predictions with belief < {value} — at the default 0.80 this catches the weak-deep cluster without firing on every parent-aware deep prediction.",
    },
    ...
    # ── audit_2026-05-06_a remediations (each independently flagged
    # so an ablation run can isolate per-remediation contribution) ──
    ...
    "classify_cautious_review_shortlist_permissive": {
        ...
        "label": "R2c — Shortlist-permissive reroute",
        "description": "When the cautious-review LLM picks a code outside the cross_subtree_belief shortlist but inside the runtime taxonomy, accept it as a shortlist-extended reroute instead of rejecting as a hallucination. Closes 8/11 errored decisions in the audit baseline.",
        "group": "convergence",
        "type": "switch",
        "default": True,
        "captions": {
            True: "Reroutes to valid taxonomy codes outside the shortlist are accepted (flagged as shortlist_extended).",
            False: "Strict closed-set: only shortlist codes accepted; LLM picks outside the 5-entry shortlist error out.",
        },
    },
    "classify_cautious_review_exclude_opaque_siblings": {
        "hocon_path": "classify.cautious_review.exclude_opaque_siblings",
        "label": "R3 — Exclude opaque-named siblings from review context",
        "description": "Drop opaque-named siblings (``col_NN``, ``var_NN``, ``dim_NN``, …) from the cautious-review prompt's sibling list. Breaks the col_04 sibling-context-poisoning class where reviewer reasons from misclassified opaque columns. Floors at 3 siblings to keep some context.",
        "group": "convergence",
        "type": "switch",
        "default": True,
        "captions": {
            True: "Opaque-named siblings filtered from reviewer context; reviewer cannot reason from possibly-misclassified anonymous columns.",
            False: "All same-table siblings included regardless of name shape.",
        },
    },
    "classify_cautious_review_stability_guard_enabled": {
        "hocon_path": "classify.cautious_review.stability_guard_enabled",
        "label": "R2a — Reroute stability guard",
        "description": "Reject cross-subtree reroutes when DST fusion + LLM already converged on the pre-review code with high LLM confidence. Within-subtree depth corrections (one code is an ancestor of the other) are unaffected — only true sideways moves get blocked. Prevents the gaming_profiles.handle failure class.",
        "group": "convergence",
        "type": "switch",
        "default": True,
        "captions": {
            True: "Cross-subtree reroutes are downgraded to ``keep`` when LLM and pre-review code agreed at high confidence.",
            False: "All reroute decisions are accepted regardless of fusion-LLM agreement.",
        },
    },
    "classify_cautious_review_stability_guard_llm_conf": {
        "hocon_path": "classify.cautious_review.stability_guard_llm_conf",
        "label": "R2a — Stability guard LLM confidence threshold",
        "description": "LLM confidence floor for the R2a stability guard. Reroutes pass through unblocked when llm_confidence < this threshold (because the LLM's pre-review code is itself uncertain).",
        "group": "convergence",
        "type": "float",
        "min": 0.50,
        "max": 0.99,
