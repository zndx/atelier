...
def _build_analysis_prompt(
    ...
    parts.append(
        "\n## Health as (target, tolerance) pairs, not binary 100%\n\n"
        "Real-world pipeline health is not 'everything at 100% or "
        "broken'.  Every health signal is reported as a triplet: "
        "**(target, actual, in-tolerance)**, where tolerance is "
        "expressed in units the operator can act on — absolute columns, "
        "batches worth, or fraction of scored rows.  A signal is "
        "'healthy' when its actual value sits within the declared "
        "tolerance of its target; it is 'degraded' when it's outside.  "
        "Do not describe a signal as a binary pass/fail when the "
        "tolerance is available.\n\n"
        "### Reference tolerance table (starting point — adjust per "
        "run when the evidence warrants)\n\n"
        "| Signal | Target | Tolerance | Tolerance expression |\n"
        "|---|---|---|---|\n"
        "| ``llm_coverage`` | 1.0 | columns not reached by the LLM "
        "≤ ``2 × classify_llm_columns_per_call`` | absolute column "
        "shortfall bounded by two batches worth — noise from "
        "individual batch failures is within tolerance; systematic "
        "shortfall is not |\n"
        "| ``llm_agreement`` (fit-to-LLM regime) | 1.0 | "
        "disagreements ≤ max(3, 1% of LLM-covered cols) | "
        "per-column diagnostic; a handful of divergences is "
        "information, a double-digit percentage is a regression |\n"
        "| ``state.failed_columns`` | empty | ≤ 2 columns OR ≤ 1% "
        "of corpus, whichever is smaller | anything remaining after "
        "halving-retry and coverage-gap retry is notable and must "
        "be named by column |\n"
        "| ``matches_reference`` coverage | scoped to the curated "
        "reference; not all columns have one | missing reference on "
        "non-curated columns is expected; missing reference on "
        "columns that DO have a curated entry is a finding |\n"
        "| Accuracy (when scored) | anchored on a shipped baseline "
        "— e.g. 94.56% for the UAT meta-tagging bundle | ± 1 pp "
        "absolute, widening to ± 2 pp when LLM temperature > 0 or "
        "batch_size changed | state the baseline you're comparing "
        "to and the sampling variance that governs the tolerance |\n"
        "\n"
        "If a deployment or dataset needs different tolerances, state "
        "them explicitly in the report's **Summary** section and "
        "proceed; do not silently adopt the table above when the "
        ...
        "columns ``target``, ``actual``, ``tolerance``, "
        "``in-tolerance?``.  Use the reference tolerance table above "
        "as the starting point; justify any deviation in one "
        "sentence.  Follow with one or two sentences naming the "
        "fusion rule and reference-filter state so the rest of the "
        "report can be read in context.\n"
    ...
    if tunable_keys:
        parts.append(
            ...
            "only when its corresponding health signal is OUT of "
            "tolerance in this run.**  If every signal in the Summary "
            "health-signals table is in tolerance, omit the block "
            "entirely — no key earns the operator's attention by "
            "default.\n\n"
            "Keep the list to the 3–7 keys most directly connected "
            "to the out-of-tolerance findings.  Use the exact keys "
            "from the tunable list below; unknown keys are silently "
            "dropped.\n\n"
            "Examples of the tolerance-gated convention:\n\n"
            "- Coverage shortfall outside tolerance (e.g. "
            "``llm_coverage=0.72`` with target 1.0 and tolerance "
            "``≤ 2 × classify_llm_columns_per_call`` violated by "
            "a systematic gap): focus on "
            "``classify_llm_columns_per_call`` (reduce batch so "
            "Bedrock's per-model ceiling doesn't silently truncate) "
            "or ``classify_llm_max_tokens``.\n"
            "- Accuracy below the declared baseline beyond the "
            "sampling-variance tolerance: focus on "
            "``classify_bootstrap_clarity_target`` and "
            "``classify_discount_llm``.\n"
            "- Every signal in tolerance: emit no ``focus`` block.\n\n"
            "```focus\n"
...
def _supervisor_system_prompt(cfg, autonomy: str) -> str:
    ...
    return (
        ...
        "didn't work.\n\n"
        "## Health as (target, tolerance) pairs\n\n"
        "Evaluate every health signal as a triplet — **target, actual, "
        "in-tolerance?** — not as binary 100%/not.  Intervene only on "
        "signals that are outside their declared tolerance.\n\n"
        "Reference tolerance table (adjust per run when evidence "
        "warrants, and state the deviation):\n\n"
        "| Signal | Target | Tolerance |\n"
        "|---|---|---|\n"
        "| `llm_coverage` | 1.0 | columns missed ≤ "
        "`2 × classify_llm_columns_per_call` (two batches worth) |\n"
        "| `llm_agreement` (fit-to-LLM) | 1.0 | disagreements ≤ "
        "max(3, 1% of LLM-covered) |\n"
        "| `state.failed_columns` | empty | ≤ 2 OR ≤ 1% of corpus |\n"
        "| accuracy-vs-baseline | declared baseline (e.g. 94.56% "
        "UAT) | ± 1 pp (widen to ± 2 pp if temperature > 0) |\n\n"
        "A signal 'inside tolerance' does not warrant an overlay "
        "change.  Propose overlays only when your evidence triplet "
        "shows a target, an actual, and a clear violation of the "
        "tolerance — the proposal's predicted outcome is then a "
        "specific movement of that actual back toward target.\n\n"
        "## Pipeline design notes (descriptive, not instructive)\n\n"
