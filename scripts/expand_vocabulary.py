...
def cat(
    ...
    Two provenance-hardening filters are applied here so the generated
    fixture cannot reintroduce customer-derived encodings (audit
    2026-04-30, see fixtures/PROVENANCE.md):

    1. Any abbrev with a ``C_`` prefix is dropped (set to empty string).
       The ``C_*`` prefix is a customer-internal class-namespacing
       convention with no public-ontology grounding; it was carried
       over from a deprecated mock_annotations.json fixture.  Class
       and parent nodes carry no abbrev in our shipped vocabularies.
    2. The ``notation`` field is always emitted empty.  The original
       autogenerated dotted-numeric values structurally coincided with
       customer code values; the universal layer carries no public-
       source numeric encoding.  The field exists in the schema only
       to carry the customer's own SKOS notation when domain
       extensions are loaded at runtime.
    """
    safe_abbrev = "" if abbrev.startswith("C_") else abbrev
    result = {
        "code": code,
        "label": label,
    ...
    # notation deliberately left out (or empty); see docstring.
    return result
...
def validate(cats: list[dict]) -> list[str]:
    ...
    for c in cats:
        ...
        # Required fields.  ``abbrev`` is required only on leaves —
        # parent nodes carry no abbrev under the post-2026-04-30
        # provenance rules (see ``cat`` docstring).  We can't tell leaf
        # vs parent from a single record here; defer that to the
        # downstream loader and only require code/label/description.
        for field in ("code", "label", "description"):
            if not c.get(field):
    ...
    # Check for duplicate abbrevs.  Empty abbrev is the expected state
    # for class/parent nodes (see ``cat`` docstring + PROVENANCE.md);
    # only check uniqueness on populated leaf abbrevs.
